...
{
    if (!s_outs_container || !s_game_state) return;

    bool show_outs = (!s_game_state->is_leg_finished && s_game_state->current_score <= 170 && s_game_state->outs_count > 0);
    ESP_LOGI(TAG, "update_outs_display: score=%ld, show_outs=%d, count=%d",
             (long)s_game_state->current_score, show_outs, s_game_state->outs_count);
    ...
    for (uint8_t i = 0; i < MAX_OUT_ROWS; i++) {
        if (show_outs && i < s_game_state->outs_count) {
            ...
            while (token != NULL && badge_idx < MAX_BADGES_PER_ROW) {
                ...
                if (strlen(token) > 0) {
                    lv_obj_clear_flag(s_out_badge_boxes[i][badge_idx], LV_OBJ_FLAG_HIDDEN);
                    lv_label_set_text(s_out_badges[i][badge_idx], token);

                    if (i == 0) {
                        lv_obj_set_style_bg_color(s_out_badge_boxes[i][badge_idx], UI_COLOR_PRIMARY, 0);
                        lv_obj_set_style_text_color(s_out_badges[i][badge_idx], lv_color_white(), 0);
                    } else {
                        lv_obj_set_style_bg_color(s_out_badge_boxes[i][badge_idx], UI_COLOR_BTN_BG, 0);
                        lv_obj_set_style_text_color(s_out_badges[i][badge_idx], UI_COLOR_TEXT_MAIN, 0);
                    }
                    badge_idx++;
                }
...
void ui_view_update(void)
{
    ...
    char score_str[32];
    snprintf(score_str, sizeof(score_str), "%ld", (long)s_game_state->current_score);
...
{
    if (s_game_state && s_game_state->is_leg_finished) return;
...
void ui_view_init(darts_game_state_t *state)
{
    ...
    lv_obj_set_style_shadow_color(left_panel, lv_color_make(200, 200, 200), 0);
    lv_obj_set_style_pad_hor(left_panel, 8, 0);
    lv_obj_set_style_pad_top(left_panel, 0, 0);
    ...
    // Main Score Display (Native Digits-Only 140pt Segoe UI Font, centered)
    s_score_label = lv_label_create(left_panel);
    lv_obj_set_height(s_score_label, 160);
    ...
    lv_obj_set_style_pad_top(s_score_label, 0, 0);

    // Active Turn Input Display
    s_input_label = lv_label_create(left_panel);
    lv_obj_set_height(s_input_label, 42);
    lv_obj_set_style_text_font(s_input_label, UI_FONT_LARGE, 0);
    lv_obj_set_style_text_color(s_input_label, UI_COLOR_PRIMARY, 0);

    // Outs Container (Max 2 rows)
    s_outs_container = lv_obj_create(left_panel);
    lv_obj_remove_flag(s_outs_container, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_width(s_outs_container, lv_pct(100));
    lv_obj_set_height(s_outs_container, 70);
    ...
    for (uint8_t i = 0; i < MAX_OUT_ROWS; i++) {
        ...
        lv_obj_set_height(s_out_rows[i], 32);
        ...
        lv_obj_set_flex_align(s_out_rows[i], LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
        lv_obj_set_style_pad_column(s_out_rows[i], 8, 0);
        lv_obj_add_flag(s_out_rows[i], LV_OBJ_FLAG_HIDDEN);
        ...
        for (uint8_t b = 0; b < MAX_BADGES_PER_ROW; b++) {
            ...
            lv_obj_set_height(s_out_badge_boxes[i][b], 28);
            lv_obj_set_style_radius(s_out_badge_boxes[i][b], 6, 0);
            lv_obj_set_style_border_width(s_out_badge_boxes[i][b], 0, 0);
            lv_obj_set_style_pad_ver(s_out_badge_boxes[i][b], 2, 0);
            lv_obj_set_style_pad_hor(s_out_badge_boxes[i][b], 10, 0);
    ...
    ui_keypad_callbacks_t keypad_cbs = {
        .on_digit = on_digit_pressed,
