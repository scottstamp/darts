...
// Standard Darts Targets in preferred setup order (Triples first, then Singles, then Doubles for finish)
static const dart_target_t ALL_TARGETS[] = {
    // Triples (High scoring setup)
    {"T20", 60, false}, {"T19", 57, false}, {"T18", 54, false}, {"T17", 51, false},
    {"T16", 48, false}, {"T15", 45, false}, {"T14", 42, false}, {"T13", 39, false},
    {"T12", 36, false}, {"T11", 33, false}, {"T10", 30, false}, {"T9", 27, false},
    {"T8", 24, false},  {"T7", 21, false},  {"T6", 18, false},  {"T5", 15, false},
    {"T4", 12, false},  {"T3", 9, false},   {"T2", 6, false},   {"T1", 3, false},

    // Singles
    {"20", 20, false}, {"19", 19, false}, {"18", 18, false}, {"17", 17, false},
    {"16", 16, false}, {"15", 15, false}, {"14", 14, false}, {"13", 13, false},
    {"12", 12, false}, {"11", 11, false}, {"10", 10, false}, {"9", 9, false},
    {"8", 8, false},   {"7", 7, false},   {"6", 6, false},   {"5", 5, false},
    {"4", 4, false},   {"3", 3, false},   {"2", 2, false},   {"1", 1, false},
    {"25", 25, false},

    // Doubles (Valid Finishes)
    {"D20", 40, true}, {"D16", 32, true}, {"D10", 20, true}, {"D8", 16, true},
    {"D12", 24, true}, {"D18", 36, true}, {"D14", 28, true}, {"D25", 50, true},
    {"D19", 38, true}, {"D17", 34, true}, {"D15", 30, true}, {"D13", 26, true},
    {"D11", 22, true}, {"D9", 18, true},  {"D7", 14, true},  {"D6", 12, true},
    {"D5", 10, true},  {"D4", 8, true},   {"D3", 6, true},   {"D2", 4, true},
    {"D1", 2, true}
};

static const size_t TARGETS_COUNT = sizeof(ALL_TARGETS) / sizeof(ALL_TARGETS[0]);
...
void darts_engine_reset(darts_game_state_t *state)
{
    ...
    state->outs_count = darts_engine_get_checkouts(state->current_score, state->outs);
}

uint8_t darts_engine_get_checkouts(int32_t score, checkout_option_t outs[MAX_OUT_COMBINATIONS])
{
    // Filter impossible checkouts immediately (score < 2, score > 170, or impossible 3-dart checkout numbers)
    if (score < 2 || score > 170 ||
        score == 169 || score == 166 || score == 165 ||
        ...
        for (size_t i = 0; i < TARGETS_COUNT && count < MAX_OUT_COMBINATIONS; i++) {
            ...
            if (d2_name != NULL) {
                ...
                for (uint8_t m = 0; m < count; m++) {
                    if (strcmp(outs[m].str, temp) == 0) { dup = true; break; }
                }
                if (!dup) {
    ...
    if (count < MAX_OUT_COMBINATIONS) {
        for (size_t i = 0; i < TARGETS_COUNT && count < MAX_OUT_COMBINATIONS; i++) {
            ...
            if (rem1 < 4 || rem1 > 110) continue;

            for (size_t j = 0; j < TARGETS_COUNT && count < MAX_OUT_COMBINATIONS; j++) {
                int32_t rem2 = rem1 - ALL_TARGETS[j].value;
                if (rem2 < 2 || rem2 > 50) continue;

                const char *d3_name = get_double_target_name(rem2);
                if (d3_name != NULL) {
                    char temp[MAX_OUT_STR_LEN];
                    snprintf(temp, sizeof(temp), "%s - %s - %s", ALL_TARGETS[i].name, ALL_TARGETS[j].name, d3_name);
                    bool dup = false;
                    for (uint8_t m = 0; m < count; m++) {
                        if (strcmp(outs[m].str, temp) == 0) { dup = true; break; }
                    }
